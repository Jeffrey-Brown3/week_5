...
public class DatabaseStockService implements StockService {
    ...
    public List<StockQuote> getQuote(String symbol, Calendar from, Calendar until) throws StockServiceException {
        ...
        List<StockQuote> stockQuotes = null;
        java.sql.Date fromDateIn = new java.sql.Date(from.getTimeInMillis());
        java.sql.Date untilDateIn = new java.sql.Date(until.getTimeInMillis());

        try {
            Connection connection = DatabaseUtils.getConnection();
            Statement statement = connection.createStatement();
            String queryString = "select * from quotes where symbol = '" + symbol + "'" +"AND time BETWEEN '" + untilDateIn + "' AND '" + fromDateIn +"'";
